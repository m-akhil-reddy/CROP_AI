...
# Import real ML module (auto-falls back to simulation if no trained model)
from ml import (
    analyze_crop_damage,
    analyze_image,
...
from ml.weather_validation import verify_weather, generate_weather_report
from ml.scoring import calculate_gps_match_score
...
# ═══════════════════════════════════════════════════════════════════════════
#  AI ANALYSIS ENDPOINT (upgraded with real ML pipeline)
# ═══════════════════════════════════════════════════════════════════════════

@app.route('/api/analyze', methods=['POST'])
def ai_analyze():
    """
    Upgraded AI analysis endpoint.
    Accepts JSON metadata and optionally uploaded images.
    Runs real CNN inference when a trained model is present,
    otherwise uses enhanced simulation.
    """
    # --- Parse input (supports both JSON and multipart/form-data) --------
    if request.content_type and 'multipart' in request.content_type:
        crop_type = request.form.get('crop_type', 'Rice')
        damage_type = request.form.get('damage_type', 'Flood')
        damage_percentage = int(request.form.get('damage_percentage', 50))
        crop_stage = request.form.get('crop_stage', 'Growing stage')
        district = request.form.get('district', '')
        land_area = float(request.form.get('land_area', 1.0))
        gps_location = request.form.get('gps_location', '')
        land_gps = request.form.get('land_gps', '')
        ...
        gps_location = data.get('gps_location', '')
        land_gps = data.get('land_gps', '')
        user_id = data.get('user_id', '')
        uploaded_images = []

    # --- 1. Image Analysis -----------------------------------------------
    if uploaded_images:
        # Save images temporarily and run analysis
        image_paths = []
        ...
        if image_paths:
            analysis = analyze_multiple_images(image_paths)
            # Clean up temp files
    ...
    # --- 2. Fraud Detection -----------------------------------------------
    fraud_result = {"fraud_probability": 8, "fraud_flags": [], "is_suspicious": False}
    if uploaded_images:
        # Re-check using the first image (already saved above is gone, use form data)
        # For fraud check, we use the image bytes from the request
        try:
            first_img = request.files.getlist('images')
            if first_img:
                fraud_result = check_fraud(
                    image_input=None,  # images already processed
                    claim_gps=gps_location,
                    land_gps=land_gps,
                    user_id=user_id,
                )
        except Exception as e:
    ...
    weather_result = verify_weather(
        gps_location=gps_location if gps_location else None,
        damage_type=analysis.get('damage_type_raw', damage_type),
        district=district if district else None,
    )

    # --- 4. GPS Matching --------------------------------------------------
    gps_result = calculate_gps_match_score(
        claim_gps=gps_location,
        land_gps=land_gps,
    )

    # --- 5. Final Eligibility Score ---------------------------------------
    eligibility = calculate_final_score(
        ai_confidence=analysis.get('confidence', 70),
        weather_score=weather_result.get('weather_score', 60),
        gps_match_score=gps_result.get('gps_score', 70),
        fraud_probability=fraud_result.get('fraud_probability', 10),
        claim_history_score=70,  # TODO: calculate from user's past claims
    )

    # --- 6. Loss Estimation -----------------------------------------------
    loss = estimate_loss(crop_type, analysis.get('damage_percentage', damage_percentage), land_area)
    ...
    return jsonify({
        ...
        "data": {
            ...
            "weather": weather_result.get('report', {}),
            "loss_estimate": loss,
            "fraud": {
            ...
            "weather_validation": {
                "weather_match": weather_result.get('weather_match', True),
                "weather_score": weather_result.get('weather_score', 60),
            },
            "model_mode": analysis.get('model_mode', 'simulation'),
        },
    })
...
def fraud_check():
    """Dedicated fraud analysis for uploaded images."""
    image_file = request.files.get('image')
    claim_gps = request.form.get('claim_gps', '') or (request.get_json() or {}).get('claim_gps', '')
    land_gps = request.form.get('land_gps', '') or (request.get_json() or {}).get('land_gps', '')
    user_id = request.form.get('user_id', '') or (request.get_json() or {}).get('user_id', '')
    claim_id = request.form.get('claim_id', '') or (request.get_json() or {}).get('claim_id', '')

    # Save image temporarily if provided
    image_path = None
    if image_file and image_file.filename:
        ext = os.path.splitext(image_file.filename)[1]
        temp_name = f"fraud_{uuid.uuid4()}{ext}"
...
# ═══════════════════════════════════════════════════════════════════════════
#  WEATHER VERIFICATION ENDPOINT
# ═══════════════════════════════════════════════════════════════════════════

@app.route('/api/weather-verify', methods=['POST'])
def weather_verify():
    """Verify weather conditions for a GPS location and damage type."""
    data = request.get_json() or {}
    gps_location = data.get('gps_location', '')
    damage_type = data.get('damage_type', '')
    district = data.get('district', '')

    result = verify_weather(
        gps_location=gps_location if gps_location else None,
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*60}")
    print(f"  AI-Based Crop Damage Assessment Server")
    print(f"{'='*60}")
    print(f"  URL     : http://localhost:{port}")
    print(f"  Static  : {os.path.abspath(app.static_folder)}")
    print(f"  Uploads : {os.path.abspath(UPLOAD_FOLDER)}")
    print(f"  ML Mode : {'Real CNN (EfficientNetB0)' if is_real_model_active() else 'Enhanced Simulation'}")
    print(f"{'='*60}\n")
